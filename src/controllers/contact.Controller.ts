...
export const Banner = async (req: any, res: any) => {
    try {
        const data = await ContactModel.findByIdAndUpdate(req.params.id, { $set: { banner: req.body.payload } });
...
// LongTermPartnership
export const LongTermPartnership = async (req: any, res: any) => {
    try {
        const data = await ContactModel.findByIdAndUpdate(req.params.id, { $set: { longterm_partnership: req.body.payload } });
